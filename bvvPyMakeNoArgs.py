     ...
     if not file_present:
      ...
      if target in changed_files:
       ...
       for dep in depend[target]:
        ...
        if dep_mod > target_mod and not os.path.isdir(dep):
         tobeupdated.append(target)
         if echo:
...
def check_bsub_success(filename, echo = False):
    """ Looking for the word 'Done' in a given *.out file
        Return a triple of boolean: (File Found, Status Found, Status Success)
    """
    ...
    while True:
        ...
        if (cnt_done > cnt_done_prev) and cnt_done != len(tobeupdated) or not first_job_started:
            ...
            for ind in safe2start[0: MAX_ADD_JOBS]:
                ...
                #command = "bsub " + "-q " + queue + " -o " + dir + "/" + tobeupdated[ind] + ".out " + "-e " + dir + "/" + tobeupdated[ind] + ".err " + script_dir + "/" + sys.argv[0] + " " + rules[tobeupdated[ind]].__name__ + " " + tobeupdated[ind] + " " + str(ind)
                #command = "bsub " + "-q " + queue + " -o " + dir + "/" + tobeupdated[ind] + ".out " + "-e " + dir + "/" + tobeupdated[ind] + ".err " + "-R " + ' "hname!=bullet" ' + script_dir + "/" + sys.argv[0] + " " + rules[tobeupdated[ind]].__name__ + " " + tobeupdated[ind] + " " + str(ind)
                command = "bsub " + "-q " + queue + " -o " + dir + "/" + tobeupdated[ind] + ".out " + "-e " + dir + "/" + tobeupdated[ind] + ".err " + " -R rhel60 " + script_dir + "/" + sys.argv[0] + " " + rules[tobeupdated[ind]].__name__ + " " + tobeupdated[ind] + " " + str(ind)
                print command
...
def dorule(dir, arg_globals):
    ...
    if  argc == 1 or argc == 4:
        if argc == 4:
            ...
            target = sys.argv[2]
            ind = sys.argv[3]
            arg_globals[fname](target)
            outfile = open(lockdir + "/make" + str(ind) + ".done", "w")
            outfile.write("done")
            outfile.close() 
            sys.exit()
    else:
        print "Wrong number of arguments ({}) to {}".format(argc, sys.argv[0])
        sys.exit(1)
    #}}}
